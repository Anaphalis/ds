...
Cell.prototype.updateBase = function(downMirror,downUpdateList){
  ...
  path.shift();
  mirror = rootDS;
  var _mirror = mirror,_rootDS = rootDS;
  //console.log('查看初始镜像',_mirror,_rootDS,rootDS,path);
  for(var i=0,l=path.length;i<l;i++){
    var field = path[i];
    if(i!=l-1){
      _mirror[field] = IM(_rootDS[field]);//这里肯定都是对象
    }else{
  ...
  this.updateList = IM._assign(updateList,downUpdateList);
  this.mirror = mirror;
...
Cell.prototype.updateNode = function(){
  ...
  var _updateNode = function(newVal,oldVal,currentPath,currentNode,parent,pathstr){//递归更新
    var ntype = isRawObject(newVal);
    var otype = isRawObject(oldVal);
    //最简单的处理方式就是：数据结构不能变
    //如果数据结构不一样，则需要大块的变更节点和记录路径
    //console.log('更新',newVal,oldVal,currentPath,currentNode,ntype,otype)
    if(ntype===false&&otype===false){//双方都是值
        parent[pathstr] = newVal;////不管一样不一样，还是要把新值给镜像，不然丢了
        return
    }else if(ntype!==otype){
      if(ntype===true){//原节点是值，新节点是对象,新节点
        ...
        _pathList.forEach((path)=>{updateList[path] = 'create'});
      }else if(otype==true){
      ...
      //处理删除
      odiffn.forEach((field)=>{
        if(isRawObject(oldVal[field])){
          var _path = currentPath.concat();
          _path.push(field);
          updateList[_path.join('.')] = 'recycle'
        }
        var _pathList = getAllPath(oldVal[field],currentPath);
        _pathList.forEach((path)=>{updateList[path] = 'recycle'});
      })
      //处理更新
      inter.forEach((field)=>{
        if(newVal[field]!==oldVal[field]){//这里是不是newVal[field]!==oldVal[field]
          currentNode[field] = {};
          var _currentPath = currentPath.concat();
          _currentPath.push(field);

          //这里，如果
          if(isRawObject(newVal[field])||isRawObject(oldVal[field])){
...
Cell.prototype.getAllPath = function(obj,path){//path是输入的相对路径,要放在数组中
  //console.log('GETALLAPTH',obj,path,isRawObject(obj))
  var paths = [];
  var path = path.concat();
  if(!isRawObject(obj))return paths;


  var _getPath = function (obj,currentPath){
    var fields = Object.keys(obj);
    fields.forEach((field)=>{

      //console.log('当前执行的field',field,isRawObject(obj[field]));
      if(isRawObject(obj[field])){
